...
- **Dashboard: Momentum Copilot brand mark + favicon.** Canonical brand PNG (`apps/web/public/momentum-icon.png`, green diamond + ascending arrow + bar-chart motif) consumed by a single `<MomentumLogo>` React component (`apps/web/components/brand/momentum-logo.tsx`) used in the sidebar header and the login header. Browser-tab favicon is served by Next 15's auto-detected metadata file `apps/web/app/icon.png` — one source of truth, no per-platform variants (apple/android intentionally omitted).
- **Dashboard: shared `PageShell` + `PageHeader` (`apps/web/components/shell/page-shell.tsx`).** Canonical app page container — left-aligned, `max-w-7xl`, consistent horizontal padding (`px-6 lg:px-10`) and vertical rhythm (`py-8`, `space-y-8`). Every `(app)/*` page (Overview, Candidates, Scan History `/scans` + `/scans/[id]`, Alert History, Settings, Assets) was migrated to it, so titles, subtitles, KPI strips, tables, and section headings now share a single grid line. Replaces the ad-hoc `mx-auto max-w-*` blocks that previously made app pages feel centred and inconsistent.
- **Dashboard: per-page browser titles.** Root layout sets a Next metadata title template (`%s — Momentum Copilot`); each page exports its own `metadata.title` (`Overview`, `Candidates`, `Scan History`, `Alert History`, `Settings`). Viewport `themeColor` set to `#0a0f1a` to match the dark surface.

### Changed

- **Alerts: New/Updated recency classification replaces dedup suppression (behaviour change).** Previously a coin alerted within `scanner.alert_dedup_hours` was **suppressed entirely** — it silently vanished from alerts until the window lapsed. Now qualifying coins are **never suppressed**: that same window is repurposed into a recency classifier. A coin with a prior sent `new_candidate` alert inside the window renders as **Updated** (with a ▲/▼ spot-price delta vs that alert's `market_snapshots` price); otherwise **New**. Clean/Ugly remains the top-level grouping and channel routing — each channel message now nests plain-text `New — X` / `Updated — Y` sections (empty sections omitted). Updated coins are sorted by delta descending, with no-baseline coins (no recoverable prior spot) shown without a delta and sorted last. Scope is per-asset, not per-channel. Every emitted alert (New or Updated) still writes an `alerts_sent` row and transitions to `alerted`. The setting key `scanner.alert_dedup_hours` is **retained** (DB/UI compatibility) but its meaning is now "recency window"; `AlertConfig.dedup_window_hours` → `recency_window_hours` internally. No schema/DDL change — the 0012 seed description **and value (8h → 24h)** were repurposed/bumped in place. The default recency window is now **24h** (seed value + `settings.py` / `alerter.py` Python defaults); existing deployments should set `scanner.alert_dedup_hours = 24` via `/settings` to apply it live, since the in-place seed edit does not re-run on an already-migrated DB. `apps/scanner/scanner/alerter.py` (+ `settings.py` docstring); docs in `state-machine.md` / `data-model.md` / `roadmap.md` updated; `test_alerter.py` reworked (recency classification, delta formatting, section omission, Updated sort).
- **Filter: `global.rsi_hard_min` default lowered 48 → 42.** 48 was excluding liquid majors at Stage 4; 42 is the intended baseline. Shipped defaults aligned across the 0012 seed, the Python dataclass defaults (`settings.py`, `filter.py`), and `docs/scoring-model.md` so fresh/reset databases and the empty-table fallback all use 42 (production was already set to 42 via the `/settings` UI).
- **Dashboard: visual polish pass — premium dark fintech aesthetic.** No information-architecture or product-behaviour changes; design tokens, components, and shared layout only.
  - **Tokens (`globals.css`, `tailwind.config.ts`).** Deepened the background (`222 47% 4%`) and card surface (`222 40% 7%`) for a more "money" dark palette; added `--surface-elevated`, `--ring`, `info` semantic colour, and `shadow-card` / `shadow-elevated` utilities; standardized `--radius` to `0.625rem` (cards/tables now use the `xl` variant). Green is reserved for active nav, positive/clean state, and primary CTAs — body and table text are no longer green-by-default.
  - **Sidebar (`components/shell/sidebar.tsx`).** Brand mark + wordmark in a 64px header; active nav state replaced the plain `bg-muted` block with a 2px left-rail green accent over `bg-muted/70`; inactive items quieter (`text-muted-foreground` only).
  - **Login (`app/login/page.tsx`).** Form moved into a `rounded-xl border bg-card shadow-elevated` card; brand mark in the header; uppercase tracked label on the input; consistent focus ring on the input and primary button. Still centred.
  - **Tables (candidates, scans, alerts, scan-detail, asset-history).** Unified chrome: `rounded-xl border-border/70 bg-card shadow-card`, `bg-muted/30` header with `tracking-[0.12em]` uppercase column labels, `divide-y divide-border/50`, single hover state `bg-muted/20`. Candidate-table zebra striping removed (premium tables don't stripe). Candidate category tabs replaced with a segmented control + green underline accent and pill counts.
  - **Badges (`scan-status-badge.tsx`, `state-badge.tsx`, alerts table badges).** Standardised to `rounded-md ring-1 ring-inset` with semantic tokens (`bull` / `bear` / `caution` / `info`) instead of inline `text-emerald-400` / `text-red-400` / etc.
  - **Overview (`app/(app)/page.tsx`).** `StatCard` redesigned — larger value (`text-3xl tracking-tight`), uppercase tracked label, accent icon only when meaningful. `FinalizedRunCard` split into header / body / footer with proper dividers and consistent metric-cell typography.
  - **Settings (`components/settings/settings-form.tsx`).** Tabs upgraded to a segmented control with a green count accent; inputs gain focus border + ring; "Save" button uses `shadow-card`.
- **Alerts: Discord-native relative timestamp in the header.** The alert header timestamp changed from a fixed UTC string (`M/D/YY, H:MM AM/PM`) to Discord's native `<t:UNIX:R>` markdown (e.g. renders as "2 hours ago"). The scanner runs on a UTC CI runner and can't know each reader's timezone, so it now emits a raw Unix instant and lets every Discord client render it locally — the relative string updates live, client-side, with no message edits, and reads naturally alongside Discord's own absolute message-date header. Fixes the prior mismatch where the body showed UTC (e.g. "10:18 PM") while Discord's header showed the reader's local time ("6:18 PM"). `apps/scanner/scanner/alerter.py` `_format_header` only; two test assertions updated to match.
- **Scoring: soften the `score_volatility` ATR cliff.** `_score_volatility` previously dropped any `atr_pct_7d < 4.0%` straight to 1/10, a step function where a 0.18pp difference (3.82% vs 4.0%) cost 6 raw points. A new intermediate band `3.5% ≤ atr < 4.0% → 4 pts` makes the score continuous through the threshold; genuinely dead volatility (`< 3.5%`) still floors at 1, and every other band is byte-for-byte unchanged. Evidence-scoped to **watchlist fidelity, not candidate manufacture**: validated against run `4a00aeac-aeb1-49b2-8752-f2a5de243237`, this promotes XDC `low_score → watchlist` (54 → 57; the soft band is worth exactly +3, volatility 1 → 4) while INJ stays watchlist (58, still < `ugly_min_score` 62), the bottom-5 weak survivors stay `low_score` (≤42), and `clean`/`ugly` counts remain 0/0 — it is arithmetically impossible for this change to create a tradeable candidate from that cohort. No threshold, selector, entry-engine, R:R, or hard-filter changes. `apps/scanner/scanner/scoring.py` only. 13 new parametrized tests pin both soft-band edges and lock every unchanged band against regression.

### Fixed

- **Scanner: disable numba JIT to stop a pandas_ta import-time crash in Docker.** The scanner crashed on startup in the OCI Docker runtime with `RuntimeError: cannot cache function 'fibonacci': no locator available for file '/usr/local/lib/python3.12/site-packages/pandas_ta/utils/_math.py'`. Both `docker-compose.yml` (OCI runtime) and `apps/scanner/docker-compose.yml` (local dev) now set `NUMBA_DISABLE_JIT=1` and `NUMBA_CACHE_DIR=/tmp/numba_cache` on the `scanner` service. The scanner does not need numba JIT acceleration, so disabling it sidesteps the import-time cache failure with no behavioural change. Documented in `deploy/oci/README.md` and the `.env.example` files. No scanner logic, scoring, indicators, alerts, schema, or systemd-timer changes.
