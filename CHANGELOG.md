...
- **Dashboard: Momentum Copilot brand mark + favicon.** Single inline-SVG brand component (`apps/web/components/brand/momentum-logo.tsx`, M-monogram with ascending arrow accent in primary green) used in the sidebar header and login header. The same mark is rendered to the browser-tab favicon via the Next 15 metadata route `apps/web/app/icon.tsx` — no static binary, no per-platform variants (apple/android intentionally omitted). Source asset `icon-1024x1024.png` lives at the repo root as a design reference; not consumed by the build.
