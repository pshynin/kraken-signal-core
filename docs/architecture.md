...
**Branding:** a single inline-SVG brand mark lives in `components/brand/momentum-logo.tsx` and is used in the sidebar header, the login header, and — via the Next 15 metadata route `app/icon.tsx` — the browser-tab favicon. There are intentionally no platform-specific icon variants (apple-icon / android-icon); one mark, one source of truth. Per-page browser titles flow through the metadata title template `%s — Momentum Copilot` declared in `app/layout.tsx`.
