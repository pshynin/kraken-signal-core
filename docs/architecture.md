...
**Layout:** the `(app)` route group wraps all authenticated pages with a shared sidebar (`components/shell/sidebar.tsx`) and renders content through a shared `<PageShell>` + `<PageHeader>` container (`components/shell/page-shell.tsx`). That container is the single source of truth for app-page width, padding, and vertical rhythm — left-aligned, `max-w-7xl`, `px-6 lg:px-10 py-8`. Every authenticated page composes through it so titles, KPI strips, and tables share one grid line. Top-level dashboard routes: `/`, `/candidates`, `/scans`, `/alerts`, `/settings`. Deep-link routes accessed via clicks: `/scans/[id]` (run detail + diagnostic panels) and `/assets/[symbol]` (per-asset state-transition history).

**Branding:** the brand mark is a single canonical PNG (`apps/web/public/momentum-icon.png`) consumed by the `<MomentumLogo>` React component (`components/brand/momentum-logo.tsx`) in the sidebar and login headers. The browser-tab favicon is the same image served via Next 15's auto-detected metadata file `app/icon.png`. There are intentionally no platform-specific icon variants (apple-icon / android-icon); one mark, one source of truth. Per-page browser titles flow through the metadata title template `%s — Momentum Copilot` declared in `app/layout.tsx`.
